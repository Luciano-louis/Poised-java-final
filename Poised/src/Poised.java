...
public class Poised {
	public static void main (String [] args)throws IOException, ParseException {
		...
		if(User_selection.equalsIgnoreCase("New Project"))
			...
			overdue();
		else if(User_selection.equalsIgnoreCase("View Pending"))
			pending();
	...
	private static void pending() {
		{
		System.out.println("The following projects have not been completed as yet or still have an amount outstanding: ");	
		try {
			File tasks = new File ("Tasks.txt");//open text file for use
			Scanner scanIn = new Scanner(tasks);						
			while(scanIn.hasNextLine()) {
				String line = scanIn.nextLine();							
				String[] TaskSplit = line.split(", ");//split Instruction and list of numbers needed
				String Total_charge = (TaskSplit[5]);
				String Paid = (TaskSplit[6]);
				double full = (Integer.parseInt(Total_charge)- Integer.parseInt(Paid));
				if(full != 0){
	...
	private static void overdue() throws ParseException {
		{
	...
	private static void finalise() {
			...
			try {
				...
				while(scanIn.hasNextLine()) {
					...
					int full = (Integer.parseInt(Total_charge)- Integer.parseInt(Paid));
					if(Finalise.equals(Project_number)){
						System.out.println("Finalised Invoice:");
						System.out.println(Customer + ", as of the time of printing this invoice the total amount owed is: R" + (full) + "\n");							
					}
			}
		}			
			catch (FileNotFoundException e) {         
...
//method displays the menu and directs based on user input
	private static String menu() {
		...
		System.out.println("'View Overdue' - To view all pverdue projects");
		System.out.println("'View pending' - To view all Incomplete projects to date");
